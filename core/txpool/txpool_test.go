...
func TestInvalidTransactions(t *testing.T) {
	...
	pool, key := setupPoolWithConfig(noL1DataFeeConfig)
	defer pool.Stop()
...
func TestTipAboveFeeCap(t *testing.T) {
	...
	pool, key := setupPoolWithConfig(eip1559NoL1DataFeeConfig)
	defer pool.Stop()
...
func TestRepricingDynamicFee(t *testing.T) {
	...
	// Create the pool to test the pricing enforcement with
	pool, _ := setupPoolWithConfig(eip1559NoL1DataFeeConfig)
	defer pool.Stop()
